...
const diagnose = async (req, res) => {
  try {
    ...
    const prompt = `You are a medical assistant. Based on the provided diagnosis or symptoms, generate a structured prescription...`;
    console.log('🔍 Sending prompt to Gemini:', prompt);
    ...
    console.log('💾 Inserting into DB with userId:', req.user.id);
    const dbResult = await pool.query(
      'INSERT INTO prescriptions (user_id, diagnosis, prescription) VALUES ($1, $2, $3) RETURNING *',
      [req.user.id, diagnosis, prescription]
    );
